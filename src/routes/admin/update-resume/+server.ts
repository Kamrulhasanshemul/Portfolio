	...
	try {
		...
		const resumeData: Partial<Content> = {
			hero: {
				...
				description:
					'Project Manager and Data Analyst with 4 years of experience in owning products and leading teams to deliver projects successfully on time. Experienced in data analysis, data visualization (including Looker Studio), budget forecasting, and growth analysis.',
			...
			contact: {
				...
				resumeUrl: currentContent?.contact?.resumeUrl
			},
			experience: [
				{
				...
				},
				{
					...
					location: 'Dhaka, Bangladesh',
					description:
						'Led end-to-end planning, research, and deployment of new tech-driven projects.',
					achievements: [
			...
			skills: {
				...
				databases: [{ name: 'PostgreSQL', level: 85 }],
				cloudAndTools: [
		...
		for (const proj of resumeData.projects || []) {
			...
			const projectPayload = {
				...
				featured_image:
					proj.image ||
					'https://images.unsplash.com/photo-1551288049-bebda4e38f71?w=800&h=600&fit=crop', // Fallback
				impact: proj.impact,
				// Default placeholders for arrays to prevent null errors in frontend
				key_features: [{ title: 'Key Feature 1', description: 'Description of key feature.' }],
				challenges_solved: [{ challenge: 'Main Challenge', solution: 'How we solved it.' }],
				results_achieved: [{ metric: 'Impact', value: '100%', description: proj.impact }]
			};
		...
		if (result.success) {
			return json({
				success: true,
				message: 'Resume updated and Project Details synced.',
				content: result.data
			});
		} else {

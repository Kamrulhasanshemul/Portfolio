...
export const POST: RequestHandler = async ({ request, locals }) => {
    ...
    try {
        ...
        if (!allowedTypes.includes(file.type)) {
            return json({ error: 'Invalid file type. Only PDF and Word documents are allowed.' }, { status: 400 });
        }
        ...
        if (error) {
            ...
            if (error.message.includes('Bucket not found')) {
                return json({ error: 'Storage bucket "resumes" not found. Please create it in Supabase.' }, { status: 500 });
            }

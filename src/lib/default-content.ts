...
// Deep-merge stored content with defaults so records saved under older
// schemas still render completely (missing keys fall back per-section).
export function normalizeContent(stored: Partial<Content> | null | undefined): Content {
	...
	return {
		...
		skills: { ...defaultContent.skills, ...stored.skills },
		experience: stored.experience ?? defaultContent.experience,
